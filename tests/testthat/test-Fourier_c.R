...
test_that("the function Fourier_c handles errors appropriately", {
  ...
  # test for residuals
  expect_error(Fourier_c(c(1, 2, 3), station11, start_ind11, end_ind11, type11, seasonal_coefs1))
  expect_error(Fourier_c(matrix(c(2, 2, 2, 2), 2, 2), station11, start_ind11, end_ind11, type11, seasonal_coefs1))

  # test for seasonal_coefs
  expect_error(Fourier_c(residuals1, station11, start_ind11, end_ind11, type11, c(1, 2, 3)))

  # test for station
  expect_error(Fourier_c(residuals1, p + 1, start_ind11, end_ind11, type11, seasonal_coefs1))

  # test for start_ind
  expect_error(Fourier_c(residuals1, station11, "Jan", end_ind11, type11, seasonal_coefs1))

  # test for end_ind
  expect_error(Fourier_c(residuals1, station11, start_ind11, 400, type11, seasonal_coefs1))

  # test for type
  expect_error(Fourier_c(residuals1, station11, start_ind11, end_ind11, "Call", seasonal_coefs1))
})
...
test_that("the function Fourier_c outputs the correct price", {
  ...
  expect_equal(length(round(Fourier11$price, 0)), 1)
  ...
  expect_equal(length(round(Fourier12$price, 0)), 1)
  ...
  expect_equal(length(round(Fourier21$price, 0)), 1)
  ...
  expect_equal(length(round(Fourier22$price, 0)), 1)
})
